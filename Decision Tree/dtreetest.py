...
class Node(object):
    ...
    def classEntropy(self):
        ...
        self.entropy = round(dt.calculateEntropy(pa, pb), 4)
    ...
    def pickBestAttr(self): 
        ...
        for attrName in self.attributes:
            if not attrName == 'Class':
                ...
                classEntGivenAttr = dt.calculateClassEntropyGivenAttr(attrName, attrValuesPos, attrValuesNeg)
                if(self.entropy - classEntGivenAttr) > 0:
                    infGainAttr[attrName] = round(self.entropy - dt.calculateClassEntropyGivenAttr(attrName, attrValuesPos, attrValuesNeg), 4)
        if len(infGainAttr):
    ...
    def postpruning(self, l, k, validation_path):
        validationSet = dt.parseLines(validation_path)
        dbest = copyTree(self)
        for i in range(1, l):
...
def dtreetest(I, O, T):  # I: AttributeSet O: Discriminating attribute T: Target Set
    src = Node(T)
    ...
    if(len(X) > 1):
        ...
        src.childNode0 = dtreetest(src.attributes, X, src.getFeatureVectorsForAttr(X, 0))
        src.childNode1 = dtreetest(src.attributes, X, src.getFeatureVectorsForAttr(X, 1))
    return src

def init(train_path, test_path, validation_path, l, k, toprint):
    featureSet = dt.parseLines(train_path)
    rootNode = dtreetest(list(dt.attributeName), None, featureSet)
    if(toprint=="yes"):
	print "The tree is:"
        rootNode.printTree(0)
    testSet = dt.parseLines(test_path)
...
if(len(sys.argv))<6:
	print "python dtreetest.py <path-to-training-set> <path-to-test-set> <path-to-validation-set> <l> <k> <to-print>"
